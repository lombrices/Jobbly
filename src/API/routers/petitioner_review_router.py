...
async def create_petitioner_review(
    ...
    return schemas.PetitionerReview.form_orm(result)
...
async def get_petitioner_review_by_id(
    ...
    petitioner_review = await petitioner_review_crud.get_petitioner_review_by_id(db=db, petitioner_id=id_petitioner_review)
    if petitioner_review == None:
        raise HTTPException(status_code=404, detail="Petitioner review not found")
    return schemas.PetitionerReview.form_orm(petitioner_review)
...
async def get_worker_reviews_by_id(
    ...
    petitioner_reviews = await petitioner_review_crud.get_petitioners_reviews_by_id_worker(db=db, id_worker=id_worker)
    if petitioner_reviews == None:
        raise HTTPException(status_code=404, detail="Petitoners reviews are not found")
    return [schemas.PetitionerReview.form_orm(petitioner_review) for petitioner_review in petitioner_reviews]